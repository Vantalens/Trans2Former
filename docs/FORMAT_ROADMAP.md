...
4. P5：建立真实插件加载器和 fixture 插件，验证插件安装、启用、执行、禁用、卸载、回滚和 processing no-network。
5. P5：把高保真增强、本地 OCR/layout/table 和 OFD 承载到插件加载器，不进入默认核心包。
6. P6：推进高保真 DOCX/PDF/PNG/JPEG 输出，必须有视觉或结构回归。
7. P6：攻坚 OFD 等政务/专业格式，按 OFD-L0 到 OFD-L4 建立样例、DocumentModel 提取、本地渲染、高保真专项和质量回归。
8. P7：桌面发布产品化，区分 Web preview、desktop dev build、desktop installer 和 plugin release。
