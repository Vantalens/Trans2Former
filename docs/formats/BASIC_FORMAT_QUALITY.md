...
- 未实际运行输出文本回读时，`textFidelity` 为 `unverified`；warning 数量不能单独证明文本保真。
...
降级说明：P0 不是程序化 PDF 引擎，不保证复杂分页、字体嵌入和印刷级排版；表格字段按列坐标定位，DOCX 段落的首行/续行缩进和左对齐制表位分别映射。固定布局 PDF 的替代字体按来源文本框宽度缩放，但无法恢复原字体字形、逐字间距和图片；仍须用真实文档检查视觉偏移。
