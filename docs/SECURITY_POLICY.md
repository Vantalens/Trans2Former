...
- 随当前 release 提供的插件补丁包只能从本地 `/plugin-patches/` asset 读取；该读取不允许扩展成远程 URL 抓取。
...
- 复制诊断信息不得默认复制用户文档内容。
- 自动测试必须阻止前端引入 `fetch`、`XMLHttpRequest`、`sendBeacon`、`WebSocket` 等网络发送路径；唯一例外是插件安装模式读取本地 `/plugin-patches/` release asset。
...
- v0.2.1：允许插件安装模式读取本地 `/plugin-patches/` release asset，并通过测试限定不得扩展为远程抓取。
