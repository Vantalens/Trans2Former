...
const tasksMain = await readFile("docs/development/DEVELOPMENT_TASKS.md", "utf8");
const tasksArchive = await readFile("docs/archive/DEVELOPMENT_HISTORY.md", "utf8");
const tasks = `${tasksMain}\n${tasksArchive}`;
const releasePlan = await readFile("docs/operations/DESKTOP_RELEASE_PLAN.md", "utf8");
...
for (const expected of ["operations/DESKTOP_RELEASE_PLAN.md", "release asset", "OFD", "core local capabilities"]) {
...
assert.equal(docsIndex.includes("operations/DESKTOP_RELEASE_PLAN.md"), true, "docs index should expose P7 release plan");
