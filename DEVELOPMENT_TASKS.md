...
1. **P9-C 转换后检验三层**：规则 diff、SSIM 视觉对比、OCR 回读检验三层组合统一写入 QualityReport，作为项目核心差异化能力落地。
2. **P9-D 高级 OCR**：接入 PaddleOCR-VL / MinerU 等本地解析模型；模型资源完全独立按需下载，明确体积、运行内存、降级路径和失败提示。
